...
export class SearchSortSpec extends PureComponent<QueryBuilderProps, State> {
  state: State = {
    selectedOption: {},
  };
  ...
  constructor(props: QueryBuilderProps) {
    super(props);
    this.selectOptions = this.buildSelectOptions(['AlphaNumeric', 'Lexicographic', 'Numeric', 'StrLen', 'Version']);
    ...
    return (
      <>
        <div className="gf-form">
          <label className="gf-form-label">Search sort spec</label>
          <div
            className={css`
              width: 300px;
            `}
        ...
        <div>
          ...
          {selectedOption.value === 'version' && (
            <Version options={builderOptions} onOptionsChange={this.onOptionsChange} />
          )}
        </div>
      </>
    );
