...
const fetchDimensionValues = async (
  ...
  try {
    ...
    const sqlQueryStr = `SELECT DISTINCT "${escapedDimensionName}" FROM "${escapedTableName}" where "${escapedDimensionName}" IS NOT NULL`;
    console.error('SQL Query for dimension values:', { escapedTableName, escapedDimensionName, sqlQueryStr });
    const sqlQuery = {
      builder: {
        queryType: 'sql',
        query: sqlQueryStr,
